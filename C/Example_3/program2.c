...
void main(void)
{
    ...
    while(1){
	    ...
	    {
		    printf("\n  Received packet: SYS: %d, COMP: %d, MSG ID: %d", msg.sysid, msg.compid, msg.msgid);
            mavlink_msg_hello_decode(&msg, &hello_msg);
            printf("\n  Received packet: TYPE: %d, 8NUM: %d, 16NUM: %d, FNUM: %f", hello_msg.type, hello_msg.int8_num, hello_msg.int16_num, hello_msg.float_num);
            break;
	    }
    }
